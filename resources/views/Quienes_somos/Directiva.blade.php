                ...
                <div class="row justify-content-center">
                    @foreach ($directiva as $miembro)
                        <div class="col-md-10 col-lg-8 mb-4">
                            <div class="directiva-card d-flex align-items-center p-3">
                                <div class="directiva-foto">
                                    <img src="{{ $miembro['imagen'] }}" alt="Imagen de {{ $miembro['nombre'] }}">
                                </div>
                                <div class="directiva-info ms-4">
                                    <div class="cargo">{{ $miembro['cargo'] }}</div>
                                    <div class="nombre">{{ $miembro['nombre'] }}</div>
                                    <div class="email">{{ $miembro['email'] }}</div>
                                    @if (str_contains($miembro['orcid'], 'xxxx'))
                                        <span class="text-muted small fst-italic">ORCID no disponible</span>
                                    @else
                                        <a href="{{ $miembro['orcid'] }}" class="btn-orcid mt-2" target="_blank">
                                            <img src="https://orcid.org/sites/default/files/images/orcid_16x16.png"
                                                class="orcid-icon" alt="ORCID"> Ver ORCID
                                        </a>
                                    @endif
                                </div>
                            </div>
